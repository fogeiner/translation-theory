...
#define ARITHMETICS_PARSER_DEBUG DEBUG(fmt("%s %s %d:%d",\
            _token.c_str(),\
            _tokenizer.getTokenDescription(_type).c_str(),\
...
private:
    ...
    void nextToken() {
        ...
        _type = _tokenizer.nextToken();
        _token = _tokenizer.getTag();
        _lineNumber = _tokenizer.getLineNumber();
        _linePosition = _tokenizer.getLinePosition();
    }
public:
    ...
    _root(NULL) {
        ...
        if (_type != Tokenizer::T_EOF) {
            throw ParseException(
                    ...
                    _token.c_str(), _tokenizer.getLineNumber(),
                    _tokenizer.getLinePosition()).c_str()
					));
        }
